...
def get_training_save_dir(exp_path, total_time, dt, itraj,train_id, what="external_forces/"):
    ### 0: cte, 1: sin(\omega t), 2: A*exp(-t/tau)
    save_dir = get_path_config(exp_path=what+exp_path,total_time = total_time, dt = dt, itraj=itraj)+"{}/".format(train_id)
    return save_dir

def pre_process_data_for_ML(times, signals):
