...
initial_parameters = np.array([10., 25.]).astype("float32")
true_parameters = np.array([100., 5.]).astype("float32")

epochs = 10
learning_rate = 1.#float(true_parameters[0]/50)
batch_size = 250
...
model.trainable_variables[0].assign(tf.convert_to_tensor([[0., 1000.]]))
model.reset_states()
preds_in=[]
model.recurrent_layer.cell.memory_states = []
for b in tqdm(batched_data):
...
history = model.craft_fit(tfsignals[:,:-1,:], batch_size=batch_size, epochs=20, early_stopping=1e-14, verbose=1)
history += model.craft_fit(tfsignals[:,:-1,:], batch_size=batch_size, epochs=20, early_stopping=1e-14, verbose=1)
history += model.craft_fit(tfsignals[:,:-1,:], batch_size=batch_size, epochs=40, early_stopping=1e-14, verbose=1)
history += model.craft_fit(tfsignals[:,:-1,:], batch_size=batch_size, epochs=40, early_stopping=1e-14, verbose=1)
...
np.save(save_path+"kk_trained", kk_trained)
np.save(save_path+"kk_untrained", kk)
np.save(save_path+"timms_trained", timms_trained)
np.save(save_path+"timms_untrained", timms)
np.save(save_path+"history_params",np.squeeze([history[k]["PARAMS"][0] for k in range(len(history))]))

kk = np.load(save_path+"kk_untrained.npy")
kk_trained = np.load(save_path+"kk_trained.npy")
timms_trained = np.load(save_path+"timms_trained.npy")
timms = np.load(save_path+"timms_untrained.npy")
history_params = np.load(save_path+"history_params.npy")
...
axins.plot(history_params[:,0],  color="green", linewidth=10, alpha=0.75, label=r'$\hat{f}_{M.L.}$',zorder=2)
axins.plot(np.ones(len(history_params[:,0]))*true_parameters[0],  linestyle="--", color="blue", linewidth=10, alpha=0.75, label=r'$f_{true}$',zorder=1)
...
#axins.plot(freqs_signal[aa:bb], spectra_signal[aa:bb], color="red", linewidth=5, alpha=0.75)
axins.plot(history_params,  color="green", linewidth=10, alpha=0.75, label=r'$\hat{f}_{M.L.}$',zorder=2)
