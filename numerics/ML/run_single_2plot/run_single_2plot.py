...
history = model.craft_fit(tfsignals[:,:tt,:], batch_size=batch_size, epochs=100, early_stopping=1e-14, verbose=1, not_split=True)
...
ax = plt.subplot(111)
ax.plot(np.squeeze([history[k]["PARAMS"] for k in range(100)]),  color="black", linewidth=10, alpha=0.75, label=r'$\hat{\omega}_{M.L.}$')
ax.plot(np.ones(100)*omega,  linestyle="--", color="blue", linewidth=10, alpha=0.75, label=r'$\omega_{true}$')
ax.set_xlabel("ML-iteration", size=40)
# ax.set_ylabel(r'$\hat{\omega}_{M.L.}$', size=60)
ax.set_ylabel("frequency estimate", size=40)
